...
const margin = { left: 90, top: 70, right: 50, bottom: 70 };

const SVG_HEIGHT = 500 - margin.top - margin.bottom;
const SVG_WIDTH = 1000 - margin.left - margin.right;

let filterData = data.map((d) => {
  d.Time = new Date(`August 11, 2020 00:${d.Time}`);
  return [d.Year, d.Time, d.Doping];
});

let legendKeys = ["Riders with Doping Allegations", "No Doping Allegations"];
let legendColors = ["red", "green"];

let minYear = new Date(filterData[0]);
let maxYear = new Date(filterData[filterData.length - 1]);

console.log(filterData);

let xScale = d3
  .scaleTime()
  .domain([d3.min(filterData, (d) => d[0]), d3.max(filterData, (d) => d[0])])
  .range([0, SVG_WIDTH]);

let yScale = d3
  .scaleTime()
  .domain([d3.max(filterData, (d) => d[1]), d3.min(filterData, (d) => d[1])])
  .range([SVG_HEIGHT, 0]);
...
svg
  ...
  .on("mouseover", function (d, i) {
    ...
    selectTooltip.style("opacity", 0.8);
    //Tooltip Visible
    tooltip.transition().duration("50").style("opacity", 1);

    let convertTime = d[1].getMinutes() + ":" + d[1].getSeconds();

    let timeYear =
      "Time: " + convertTime.toString() + "<br>Year: " + d[0].toString();
  ...
  .on("mouseout", function (d, i) {
    //De-Highlight selected dot
    let selectTooltip = d3.select(this);
    selectTooltip.style("opacity", 1);
    //Tooltip Hidden
    tooltip.transition().duration("50").style("opacity", 0);
  });

//Scatterplot Legend
let legendScale = d3.scaleOrdinal().domain(legendKeys).range(legendColors);
